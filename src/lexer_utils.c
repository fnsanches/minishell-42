...
static int count_words(const char *s, char *set, int count)
{
    int q[2];
    int i;
...
static char **fill_array(char **aux, const char *s, char *set, int i[3])
{
    int q[2];
...
char **split_quotes(const char *s, char *set)
{
    ...
    if (!s)
        return (NULL);
    nwords = count_words(s, set, 0);
    if (nwords == -1)
        return (NULL);
    result = ft_calloc(sizeof(char *), (nwords + 1));
    if (result == NULL)
        return (NULL);
    result = fill_array(result, s, set, i);
    result[nwords] = NULL;
    return (result);
}